...
namespace Renderer {
		...
		, m_height(height)
	{
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrapMode); // _S => X
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapMode); // _T => Y
	...
	void Texture2D::addSubTexture(std::string name, glm::vec2& leftBottomUV, glm::vec2& rightTopUV)
	{
